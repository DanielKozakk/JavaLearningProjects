...
public class Songs {
    ...
    public static List<Song> getRockSongs() {

        return getSongs().stream().filter(song -> song.getGenre().contains(ROCK_GENRE) || song.getGenre().contains(ROCK_GENRE.toLowerCase())
        ).toList();
    }

    public static List<String> getGenres(){
        return getSongs().stream().map(Song::getGenre).distinct().toList();
    }
}